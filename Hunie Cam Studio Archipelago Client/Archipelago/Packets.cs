...
namespace HunieCamStudioArchipelagoClient.Archipelago
{
    ...
    public class RoomUpdatePacket
    {
        ...
        public void update()
        {
            if (checked_locaions != null)
            {
                ...
                {
                    if (!HunieCamArchipelago.curse.connected.checked_locations.Contains(i))
                    {
                        HunieCamArchipelago.curse.connected.checked_locations.Add(i);
                    }
                }
            }
            if (hint_points != null)
            {
                HunieCamArchipelago.curse.connected.hint_points = hint_points;
            }
        }
    }
    ...
    public class JsonMessagePart
    {
        ...
        public string print()
        {
            if (type == "player_id")
            {
                for (int i = 0; i < HunieCamArchipelago.curse.connected.players.Count(); i++)
                {
                    if (HunieCamArchipelago.curse.connected.players[i].slot.ToString() == text)
                    {
                        return HunieCamArchipelago.curse.connected.players[i].name;
                    }
                }
            }
            ...
            else if (type == "item_id")
            {
                ...
                for (int i = 0; i < HunieCamArchipelago.curse.connected.players.Count(); i++)
                {
                    if (HunieCamArchipelago.curse.connected.players[i].slot == player)
                    {
                        playername = HunieCamArchipelago.curse.connected.players[i].name;
                        break;
                    }
                }
                string game = "";
                foreach (KeyValuePair<int, NetworkSlot> v in HunieCamArchipelago.curse.connected.slot_info)
                {
                ...
                return HunieCamArchipelago.curse.data.data.games[game].idtoitem[Convert.ToInt32(text)];
            }
            ...
            else if (type == "location_id")
            {
                ...
                return HunieCamArchipelago.curse.data.data.games[game].idtolocation[Convert.ToInt32(text)];
            }
    ...
    public class GameData
    {
        ...
        public void setup(string game)
        {
            ...
            foreach (KeyValuePair<string, long> i in item_name_to_id)
            {
                ...
                {
                    ArchipelagoConsole.LogError($"ERROR IN {game}'s DATAPACKAGE. ITEM ID DUIPLICATION WITH ID: {i.Value}");
                }
            }
            foreach (KeyValuePair<string, long> l in location_name_to_id)
            {
                ...
                {
                    ArchipelagoConsole.LogError($"ERROR IN {game}'s DATAPACKAGE. LOCATION ID DUIPLICATION WITH ID: {l.Value}");
                }
            }
            idtoitem = tmpi;
            idtolocation = tmpl;
